...
# https://yanwei-liu.medium.com/gradient-reversal-layer-implementation-in-pytorch-54f7d66fd033
class GradientReversalLayer(Function):
    def __init__(self):
        super(GradReverse, self).__init__()
    
    @ staticmethod
    def forward(ctx, x, theta=1):
...
# self=daae
# https://github.com/fungtion/DANN/blob/master/
class DomainAdversarialAutoencoder(nn.Module):
    def __init__(self, structure):
        super(DomainAdversarialAutoencoder, self).__init__()
        encoder_neurons, decoder_neurons, classifier_neurons = structure
...
class DAAE():
    def __init__(self, config, X_train, D_train):
        ...
        domain_size = D_train.shape[1]
        self.structure = config.get_structure(input_size, domain_size)
    ...
    def _load_batches(self, batch_size=100, shuffle=True):
        ...
        for i in range(int(np.ceil(idx.size/batch_size))):
            ...
            self.D_train.iloc[idx[i*batch_size:(i+1)*batch_size]]
    ...
    def _train_one_epoch(self):
        ...
        batch_losses = []
        for x, d in self._load_batches(self.batch_size, self.shuffle):
            ...
            # Forward pass
            reconstruction_ouputs, domain_ouputs = self.model(x)
        ...
        epoch_loss = dict(zip(['reconstruction loss', 'domain loss', 'total loss'], epoch_loss))
    ...
    def train(self):
        ...
        self.model = DomainAdversarialAutoencoder(self.structure)
        ...
        for epoch in range(self.n_epochs):
            ...
            if epoch%50==0 or epoch==self.config.n_epochs-1 or stop:
                # Save
                history.to_csv(self.config.training_path+'history.csv', index=False)

                # Save the trained model (fix chinese path issue)
                with open(self.config.model_path, mode='wb') as f:
                ...
                plt.title(title)
                plt.xlabel('epoch')
                plt.savefig(self.config.training_path+title+'.png', transparent=True, 
                            bbox_inches='tight', dpi=144)
                # with warnings.catch_warnings():
                #     warnings.filterwarnings("ignore", category=UserWarning)
                #     plotly.offline.plot(tls.mpl_to_plotly(fig), 
                #                         filename=self.config.training_path+title+'.html', 
                #                         auto_open=False)
                plt.clf()
                plt.close('all')
    ...
    def load_model(self):
        model = DomainAdversarialAutoencoder(self.structure)
        with open(self.config.model_path, mode='rb') as f:
    ...
    def predict(self, x):
        ...
        reconstruction_ouputs, _ = self.model(x)
        reconstruction_ouputs = reconstruction_ouputs.detach().numpy()
        return reconstruction_ouputs
